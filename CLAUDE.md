...
- POST /api/incentive-plans/ocr - 시책안 이미지/PDF(multipart) → R2 원본 보관(SHA-256 멱등) → 업로드 즉시 incentive_plans 대장 등록(F-048) → CLOVA OCR + Upstage 구조화 → 시책룰 필드 후보 + 저신뢰 표시. OCR 결과로 대장 갱신(ocr_status/신뢰도/정산월 자동파싱), 실패도 status=failed로 기록. 하이브리드 엔진, blended 신뢰도(LLM×OCR평균, 임계 0.85). 인증 필수 (F-043/F-046/F-048)
