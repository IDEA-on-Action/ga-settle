...
- POST /api/incentive-plans/ocr - 시책안 이미지/PDF(multipart, category 대분류 필수) → R2 원본 보관(SHA-256 멱등) → 업로드 즉시 incentive_plans 대장 등록(F-048, 대분류 저장 F-051) → CLOVA OCR(10p 초과 PDF는 ≤10p 청크 분할·병합 F-049) + Upstage 구조화(JSON 강제+400 fallback, >8,000자 청크 분할·병합, 비-JSON 1회 재시도 F-059) → 시책룰 필드 후보 + 저신뢰 표시. OCR 결과로 대장 갱신(ocr_status/신뢰도/정산월 자동파싱), 실패도 status=failed로 기록. category 미선택/오값 400. 하이브리드 엔진, blended 신뢰도(LLM×OCR평균, 임계 0.85). 인증 필수 (F-043/F-046/F-048/F-049/F-051/F-059)
