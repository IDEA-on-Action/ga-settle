...
- GET /api/uploads - 최근 업로드 목록(id,원수사명,정산월,상태,카운트) 선택기용, 민감정보 제외 (F-036)
...
- POST /api/family/detect · /:id/confirm · /:id/release · GET /api/family - 가족계약 감지 HITL. 확정자(confirmedBy)는 인증 사용자 자동 (F-011/F-038)
...
- POST /api/runs/:id/adjustments · GET - 수동 보정(reason 필수)+감사로그. 등록자(createdBy)는 인증 사용자 자동 기록, approvedBy는 선택적 이중 승인자 (F-015/F-038)
