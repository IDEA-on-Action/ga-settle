...
- GET / · /demo - 고객 데모 랜딩/인터랙티브 페이지 (2026-07-11 사용자 요청으로 루트 원복). 정식 SPA는 B-006
- GET /portal · /overview · /status · /deliverables - 고객 협업 포털 공개 레이어(서버 렌더, F-054). 라이프사이클(계약·개발·검수·운영)·개요·진행현황(마일스톤)·산출물 목록. IA: docs/specs/portal-ia. (루트 아님, /portal에서 서빙)
...
- POST /api/incentive-plans/ocr - 시책안 이미지/PDF(multipart, category 대분류 필수) → R2 원본 보관(SHA-256 멱등) → incentive_plans 대장 등록(F-048, 대분류 F-051) → **비동기 큐 발행 후 즉시 202+{planId,jobId,status:queued}(F-066, 재업로드 시 이전 실패사유 클리어)**. 실제 OCR은 Queue consumer가 처리: CLOVA OCR(10p 초과 PDF는 ≤10p 청크 분할·병합 F-049) + Upstage 구조화(JSON 강제+400 fallback, **청크 2,000자 F-065**, 비-JSON 1회 재시도 F-059, **호출당 60s 타임아웃+재시도 F-065**) → 결과 R2(`{r2Key}.ocr.json`) 저장 + 대장 갱신(ocr_status/신뢰도/정산월). **실패 시 stage(clova|upstage|parse|unknown)+사유 대장 기록(F-064)**. category 미선택/오값 400. blended 신뢰도(LLM×OCR평균, 임계 0.85). 인증 필수 (F-043/F-046/F-048/F-049/F-051/F-059/F-064/F-065/F-066)
- GET /api/incentive-plans/:id/ocr-result - 큐가 저장한 OCR 구조화 결과(R2) 조회. 프론트가 job(GET /api/jobs/:id) done 확인 후 rule/payoutRows 후보 표시(F-066 비동기 응답 대체). 인증 필수 (F-066)
- GET /api/incentive-plans - 시책안 등록 대장 목록(파일명·원수사·정산월·OCR상태·신뢰도·업로더·시각, **실패 stage/사유 포함 F-064**). 원수사명 조인, ?q·?limit·?offset+total, `{items,total}`. UI: 업로드 화면 '시책안 문서(OCR)' 탭 '시책안 등록 내역' 섹션 (F-048)
- GET /api/incentive-plan-definitions · /summary - 시상정의 카탈로그(원수사가 준 정의 원형, 무손실 17열, 만기기간 포함 F-060). ?insurerId·?month·?q(상품·납입기간·만기기간·지급시점·원수사)·?category(대분류 필터 F-056/057/058: 생보 파생(channel, 문서 docs/reference/sengbo-대분류-매핑.md), 손보설계사/자체=OCR 링크 우선+xlsx 시상유형 소급, 규칙 문서 docs/reference/sonbo-시상유형-대분류-매핑.md)+페이지. 만기기간(maturity_term): 상품·납입기간 같아도 만기별 지급율 상이(ABL생명) 구분(F-060). incentive_rules(정산엔진)와 분리, 정의는 참조/후보 (F-044/F-056/F-057/F-058/F-060)
...
- GET /api/deliverables · GET /api/deliverables/:code/file - 산출물 대장 목록(메타)·마크다운 다운로드. SoT는 docs/deliverables/README.md, `scripts/gen-portal-deliverables.mjs`가 `portal-deliverables.gen.ts` 생성(대장/문서 수정 시 재실행, content-currency-check가 drift 감지). 포털 /deliverables '프로젝트 문서' 섹션은 메타 공개, 파일은 인증 필수 (F-070)
