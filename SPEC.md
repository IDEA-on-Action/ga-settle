...
- **Status**: 📋 PLANNED
- **Sprint**: S20
- **Notes**: 2026-07-11 등록. 요청: `https://ata.minu.best/` IA 구조 설계. 계약이 아직 미체결 상태라 이 F-item은 **IA 설계 산출물까지**(구현은 계약 확정 후 후속 F-item으로 분리 - 260711 고객 안내 메일 "추가 개발은 계약 후" 명시분과 정합). 성격: Feature(설계 단계). 산출물 위치 후보 `docs/specs/portal-ia/`. 접근 모델 공개+일부보호는 WAF(Super Bot Fight Mode) 경계·인증 게이트(F-024)와 함께 검토([[cloudflare-worker-custom-domain-waf]]). 구현 범위는 설계 확정 후 재산정.
