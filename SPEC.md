...
| ~~B-014~~ | ~~배포 자동화(ci.yml deploy 잡 + D1 migration)~~ -> **구현 완료(2026-07-17), secret 설정 대기**. **실체 정정**: F-064 때 "migration 갭"으로 봤으나 실제는 **배포 자동화 자체가 없었음**(ci.yml=verify만, 배포는 수동 `wrangler deploy`). F-064/065/066이 CI green인데 07-17 07:51까지 prod 미배포 방치되다 수동 일괄 배포로 확인. fix: ci.yml에 `deploy` 잡(verify 후 main push, migration→web build→deploy→헬스 스모크, `d1_migrations` 자동 기록으로 drift 해소). **잔여(사용자, 2026-07-17)**: secret 추가됨 → 자동화 배선 작동 확인(가드 통과·migration 스텝 실행). 단 **토큰 권한 부족으로 migration 스텝 실패(code 7403)**. 필요: (1) `CLOUDFLARE_ACCOUNT_ID`=`02ae9a2bead25d99caa8f3258b81f568`(생각과 행동) 확인 (2) `CLOUDFLARE_API_TOKEN`에 **Account>D1>Edit + Account>Workers Scripts>Edit + Zone>Workers Routes>Edit**(커스텀 도메인 ata.minu.best) 권한 부여. 프로덕션 코드는 수동 배포분(`3eff90f7`)이 정상 가동 중이라 무장애 | 토큰 권한 대기 | - |
