...
| B-012 | OCR 시책안 정식 구현 잔여 - 정의→운영룰 확정 UI(정산 반영) + 감사 소명 화면 (스키마 확장·OCR→정의 write·손보 318열 구조화는 F-044로 완료) | 다수 파일 | high |
