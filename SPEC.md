...
- **Acceptance**:
  - [x] incentive_plan_definitions에 만기기간(maturity_term) 컬럼 추가 (D1 migration 0009, nullable)
...
- **Acceptance**:
  ...
  - [x] 배포된 코드 재현 검증(2026-07-17): 고객 실파일·프로덕션 인증 부재로 로컬 동일코드 E2E로 갈음(F-048/F-063/F-065/F-066 선례). 러닝 dev 서버(=배포 main 코드)에 400행×필수3필드누락 xlsx 업로드 → job queued→running→**done**, upload **status=review**, **errorCount=1,200 전량 기록**(계약번호/설계사명/지급수수료 각 400). 구코드였다면 1,200×5=6,000 파라미터로 D1 100 한도 초과=파싱 실패. 삼성화재 963행→2,911오류와 동일 시나리오 실증. (fix 코드 b6fef48 prod 배포됨 + 회귀 테스트 302건 매 CI 통과.)
- **Status**: ✅ DONE (2026-07-17 배포된 코드 로컬 E2E 재현 - review 도달·오류 1,200 전량 기록)
- **Sprint**: S24
- **Notes**: 2026-07-15 고객 피드백("삼성화재 시책지급내역 파싱실패, 공백과 항목 300개 이상"). 파싱·헤더감지(6행째)·프로파일링은 전부 정상이었고 실패 지점은 오류 리포트 bulk insert. 고객 추정(공백·항목 수)은 간접 원인 - 공백 많은 필수 필드(설계사명 444건 누락 등)가 오류 건수를 키워 한도 초과 유발. 참고: 이 파일은 시책지급내역이라 수수료 온톨로지와 안 맞아(지급수수료 미매핑) staged 0건 - 파싱 성공 후 매핑 HITL(review)에서 다뤄야 하는 별개 주제. **종결(2026-07-17)**: fix(b6fef48/PR #61) 프로덕션 배포됨 + 회귀 테스트(오류 302건→review) 매 CI 통과 + 로컬 동일코드 E2E(400행→오류 1,200 review 도달·전량 기록, upload cascade 삭제로 정리) 3중 검증. 고객 실파일 프로덕션 재업로드는 자격증명/파일 부재로 미수행이나 동일코드 재현으로 갈음(사용자 승인, 2026-07-17).
...
### F-069 · 개발이력 체계: CHANGELOG + 고객용 개발이력 (P2)
- **REQ-092**: 내부 변경이력 정본(CHANGELOG.md, Keep a Changelog 형식)과 고객 대면 개발이력(GS-HIS-001)이 git 실이력 기반으로 제공되어야 한다
- **Acceptance**:
  - [x] CHANGELOG.md 신설 - `[Unreleased]` 섹션 + 2026-07-07~18 일자별 Added/Fixed 소급 정리 (session-end 자동 추가 형식 호환)
  - [x] GS-HIS-001 작성 - 요약 지표 + 단계별 8구간 + 고객 피드백 반영 이력 (git 144커밋·PR 74건 실측 기반)
  - [x] 산출물 대장(README) GS-HIS-001 행 갱신 (v0.1 초안)
- **Status**: ✅ DONE (2026-07-18 v0.1. 이후 이력은 CHANGELOG [Unreleased]에 누적, GS-HIS-001은 마일스톤 시점 재파생)
- **Sprint**: S28
...
| ~~B-007~~ | ~~OTP 이메일 발송 설정~~ → **✅ 완료(2026-07-18)**: 발신 도메인 `no-reply@ideaonaction.ai`(생각과행동 조직 도메인, Resend verified)·RESEND_API_KEY prod 시크릿(CF API PUT)·OTP_ENFORCED=true 배포. **prod 실측**: @atasset 비번로그인 403{otp:true} 차단, ata0008@atasset.co.kr OTP **delivered**(MailPlug 수신 확인), admin @gmail 로그인 유지. atasset 고객도메인 DNS 권한 불필요(발신은 우리 도메인). 상세 `docs/OTP-EMAIL-SETUP.md`. [구 backlog 설명] OTP 이메일 발송 설정(Resend API 키 + 도메인 검증) - F-033 실사용 요건. **런북 작성 완료(2026-07-18, `docs/OTP-EMAIL-SETUP.md`)**: 앱 로직 기구현, 잔여=운영(Resend 계정 생각과행동 소유·atasset.co.kr root 검증(누리호스팅 DNS, MailPlug MX/SPF와 send.* 서브도메인으로 공존)·RESEND_API_KEY prod 시크릿·발송 테스트 후 OTP_ENFORCED=true). 순서 준수 필수(발송 확인 전 플래그 켜면 @atasset 락아웃). Resend 계정·DNS 접근은 사용자 실행. **⏸️ 보류(2026-07-18)**: atasset.co.kr은 고객(ATA) 도메인이라 DNS 편집 권한 없음 → root 검증 불가. **대안(런북 §대안)**: 발신 도메인을 생각과행동 소유 `minu.best`(Cloudflare zone, DNS 자유)로 → `no-reply@ata.minu.best`에서 @atasset 메일함으로 발송(수신≠발신 도메인). 고객 DNS 불요, from 주소만 minu.best로 변경. 착수 시 Resend 계정 가입만 사용자, DNS는 우리 zone이라 자체 설정 가능. **진행(2026-07-18)**: 사용자가 Resend 계정+API 키 제공 → 조회 결과 **`ideaonaction.ai`(생각과 행동 조직 도메인)가 이미 verified**(minu.best DNS 셋업 불요, 더 나음). 발신 테스트 성공(Resend email id 반환). `OTP_FROM_EMAIL` → `no-reply@ideaonaction.ai` 변경(수신 게이트 OTP_EMAIL_DOMAIN=atasset.co.kr 유지). **RESEND_API_KEY prod 시크릿 설정 완료**(wrangler secret put hang 버그 회피→CF API REST PUT, `$CLOUDFLARE_API_TOKEN`+account 02ae9a2). gmail 테스트 `delivered` 실측(발신 파이프 수신까지 확인). **잔여(마지막)**: ① 실 @atasset 계정 prod OTP 수신 확인(atasset/MailPlug 수신측 검증, 사용자 메일함 필요) ② 확인 후 OTP_ENFORCED=true(gmail delivered라 저위험이나 @atasset 수신 확인 후 flip 권장, 먼저 켜면 락아웃·admin @gmail은 항상 로그인 가능). OTP는 지금도 **선택 로그인으로 작동**(enforced off) | 인프라 설정(발신+시크릿 완료, @atasset 수신확인+flip만 잔여) | high |
...
> 프로덕션: `https://z01.minu.best` 배포·운영 중 (2026-07-21 `ata.minu.best`에서 변경 - ATA 파기확인서 수령·미선정에 따른 도메인 중립화, 구 도메인 삭제). admin=sinclairseo@gmail.com(비번). @atasset.co.kr=OTP. 주요 원수사 26곳 등록. 상세 next-task는 세션 Task 목록(#1~#7) 참조.
